...
| **"Forbidden" on `/install`** | The `install/` directory exists on disk, and an older `.htaccess` served directories instead of routing them, so the installer could never be reached. Upload the current `.htaccess`. |
| Update fails with "Could not replace `.user.ini`" | The hosting panel owns that file — cPanel marks it immutable so a site cannot override the PHP settings it hands out. Current versions never replace it: it is written only when missing. Until this version is installed, remove the lock in File Manager or delete the file and re-run the update. |
