...
| `updater.php` | Which files an update may touch: the server's own data stays protected, `.user.ini` is written when missing but never replaced (control panels lock it, and an update that insisted on replacing it rolled the release back), application files stay replaceable, and a failed write reports owner, permissions and the user PHP runs as |
